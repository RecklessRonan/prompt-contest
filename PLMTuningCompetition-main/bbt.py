...
parser.add_argument("--parallel", action='store_true',
                    help='Whether to allow parallel evaluation')
parser.add_argument(
    "--inference_framework",
    default='pt',
    type=str,
...
if inference_framework not in ['pt', 'ort']:
    raise ValueError(
        f'inference_framework only supports "pt", "ort", got `{inference_framework}` instead.')
if inference_framework == 'ort':
    assert onnx_model_path is not None, 'Path to onnx model is required, got None instead.'
    assert os.path.exists(
        onnx_model_path), f'In valid onnx model path `{onnx_model_path}`'
...
if task_name in ['SST-2', 'yelpp', 'mrpc']:
    num_labels = 2
...
class LMForwardAPI:
    def __init__(self, model_name='roberta-large', n_prompt_tokens=50, task_name='SST-2', loss_type='hinge', init_prompt_path=None):
        if model_name in ['roberta-large']:
            ...
            self.model.lm_head.bias = torch.nn.parameter.Parameter(
                torch.zeros(self.config.vocab_size))
        else:
        ...
        if cat_or_add == 'cat':
            ...
            if init_prompt_path is not None:
                ...
                self.init_prompt = torch.load(
                    init_prompt_path).weight.cpu().reshape(-1)
            else:
                print(
                    'Initial prompt embedding not found. Initialize to zero embedding.')
                self.init_prompt = torch.zeros(
                    n_prompt_tokens * self.config.hidden_size)
            print('Shape of initial prompt embedding: {}'.format(
                self.init_prompt.shape))
        else:
        ...
        if random_proj == 'normal':
            ...
            print('[Embedding] mu: {} | std: {} [RandProj]  mu: {} | std: {}'.format(
                mu_hat, std_hat, mu, std))
        ...
        if task_name == 'SST-2':
            self.metric = SST2Metric(
                target='labels', pred='logits', tokenizer=tokenizer)
            self.metric_key = 'acc'
            self.metric_name = 'SST2Metric'
        elif task_name == 'agnews':
            self.metric = AGNewsMetric(
                target='labels', pred='logits', tokenizer=tokenizer)
            self.metric_key = 'acc'
            self.metric_name = 'AGNewsMetric'
        elif task_name == 'yelpp':
            self.metric = YelpPMetric(
                target='labels', pred='logits', tokenizer=tokenizer)
            self.metric_key = 'acc'
            self.metric_name = 'YelpPMetric'
        elif task_name == 'mrpc':
            self.metric = MRPCMetric(
                target='labels', pred='logits', tokenizer=tokenizer)
            self.metric_key = 'acc'
            self.metric_name = 'MRPCMetric'
        elif task_name == 'snli':
            self.metric = SNLIMetric(
                target='labels', pred='logits', tokenizer=tokenizer)
            self.metric_key = 'acc'
            self.metric_name = 'SNLIMetric'
        elif task_name == 'trec':
            self.metric = TRECMetric(
                target='labels', pred='logits', tokenizer=tokenizer)
            self.metric_key = 'acc'
    ...
    def calc_metric(self, logits, target):
        ...
        if self.metric_key == 'acc':
            ...
            raise KeyError(
                f'[Metric] Only support [acc, f1], got {self.metric_key} instead.')

        if self.loss_type == 'hinge':
            loss = hinge_loss(logits, converted_target,
                              margin=self.margin, reduction='sum').item() / len(target)
            ...
            raise KeyError(
                f'[Loss] Only support [hinge, ce, perf], got {self.loss_type} instead.')
    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        if test_data is None:
            # batch size of dev data is the orignal batch size of training data
        ...
        if isinstance(prompt_embedding, list):  # multiple queries
            ...
            for pe in prompt_embedding:
                ...
                pe_list.append(
                    z.reshape(n_prompt_tokens, -1).repeat(bsz, 1, 1))
            # num_workers*bsz x prompt_len x dim
            ...
            prompt_embedding = prompt_embedding.reshape(
                n_prompt_tokens, -1).repeat(bsz, 1, 1)
        else:
            raise ValueError(
        ...
        if isinstance(test_data, DataSet):
            if prompt_embedding.shape[0] > bsz:
                raise ValueError(
                    'Provide a single prompt embedding for testing.')
            ...
            if parallel:  # we have multiple queries
                ...
                for i in range(len(logits) // bsz):
                    ...
                    tmp_loss, tmp_perf = self.calc_metric(
                        tmp_logits, tmp_target)
            ...
            if self.num_call % self.eval_every == 0:
                ...
                dev_loss, dev_perf = self.calc_metric(
                    logits, dev_data['labels'])
...
data_bundle = DataLoader[task_name](
    tokenizer=tokenizer, n_prompt_tokens=n_prompt_tokens).my_load(['train', 'dev'], seed)

train_data, dev_data = data_bundle.get_dataset(
    'train'), data_bundle.get_dataset('dev')
...
if parallel:
    ...
    train_data['attention_mask'] = train_data['attention_mask'].repeat(
        es.popsize, 1)
...
torch.save(model_forward_api.linear(torch.tensor(model_forward_api.best_prompt,
           dtype=torch.float32)), f=f'./results/{task_name}/{seed}/best.pt')
